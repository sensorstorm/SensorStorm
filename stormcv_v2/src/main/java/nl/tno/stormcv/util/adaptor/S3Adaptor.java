...
public class S3Adaptor implements FileAdaptor {
	...
	private S3Adaptor(String s3Key, String s3Secret, URI s3URI){
		this.s3Key = s3Key;
	...
	public S3Adaptor s3Key(String key){
		this.s3Key = key;
		return this;
	}
	
	public S3Adaptor s3Secret(String secret){
		this.s3Secret = secret;
		return this;
	}
	
	@Override
	public S3Adaptor setExtensions(String[] extensions) {
	...
	public InputStream getAsStream() throws IOException {
		...
		String bucket = bucketKey[0];
		String key = bucketKey[1];
		return s3.getObject(new GetObjectRequest(bucket, key)).getObjectContent();
	}
	...
	public FileAdaptor deepCopy() {
		return new S3Adaptor(s3Key, s3Secret, s3URI);
	}
