...
public class ChannelManager implements Serializable {
	...
	public ChannelManager(String channelId, Class<? extends Batcher> batcherClass,
			Class<? extends BatchOperation> batchOperationClass, @SuppressWarnings("rawtypes") Map conf,
	...
	public ChannelManager(String channelId, Class<? extends SingleOperation> operationClass,
			@SuppressWarnings("rawtypes") Map conf, EmitParticleInterface emitParticleHandler) {
	...
	public List<Particle> processParticle(Particle particle) {
		...
		if (operation == null) {
			try {
				...
				logger.error("For channel " + channelId + ": can not create the operation (" + operationClass.getName()
						+ ") msg=" + e);
				return null;
			}
		}
		...
		if (particle instanceof MetaParticle) {
			...
			logger.warn("For channel " + channelId + ": unknown particle type (" + particle.getClass().getName()
					+ ") to process");
			return null;
		}
	 ...
	 */
	public static List<Class<? extends DataParticle>> getOutputDataParticles(Class<? extends Operation> operationClass) {
	 ...
	 */
	public static List<Class<? extends MetaParticle>> getOutputMetaParticles(Class<? extends Operation> operationClass) {
